...
class ReplyTopicTestCase(TestCase):
  ...
  def setUp(self):
    ...
    self.username = 'jairus'
    self.password = '123abc'
    user = User.objects.create_user(username=self.username, email='jairus@email.com', password=self.password)
    self.topic = Topic.objects.create(subject='Hiyah!', board=self.board, starter=user)
    Post.objects.create(message='hiyah again', topic=self.topic, created_by=user)
...
class SuccesfulReplyTopicTests(ReplyTopicTestCase):
  def setUp(self):
    super().setUp()
    self.client.login(username=self.username, password=self.password)
    self.response = self.client.post(self.url, {'message': 'hiyah!'})
